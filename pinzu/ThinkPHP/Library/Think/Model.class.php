...
class Model
{
    ...
    public function __call($method, $args)
    {
        if (in_array(strtolower($method), $this->methods, true)) {
        ...
        } elseif (strtolower(substr($method, 0, 9)) == 'getlistby') {
            // 根据某个字段获取记录
            $field = parse_name(substr($method, 9));
            $where[$field] = $args[0];
            return $this->where($where)->select();
        } elseif (strtolower(substr($method, 0, 9)) == 'getpageby') {
            // 根据某个字段获取记录
            $field = parse_name(substr($method, 9));
            ...
            return $this->where($where)->page($args[1], $args[2])->select();
        } elseif (strtolower(substr($method, 0, 10)) == 'getcountby') {
            // 根据某个字段获取记录的某个值
    ...
    public function fields()
    {
        ...
        return $this->field(implode(",", $fields));
    }
    ...
    {
        $data = array(C_REMOVED => 1);
        return $this->where(array(C_ID => $id))->save($data) > 0;
    }

    public function setSatisfy($id)
    {
        $data = array(C_SATISFY => 1);
        return $this->where(array(C_ID => $id))->save($data) > 0;
    }
}
