    ...
    <ul class="nav nav-list">
        ...
        <div class="col-md-6">
            <table id="cMain" class="table table-bordered">
                ...
                <tbody>
                <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i; if($item["status"] == 0): ?><tr class="info">
                            <?php elseif($item["status"] == 1): ?>
                        <tr class="success"><?php endif; ?>
                    <td><?php echo ($item["id"]); ?></td>
                    <td><?php echo ($item["title"]); ?></td>
                    <td><?php echo ($item["published_on"]); ?></td>
                    <td><?php echo ($item["status_name"]); ?></td>
                    <td>
                        <button type="submit" class="btn btn-warning btn-xs">
                            查看详情
                        </button>
                        <buttton type="button" class="btn btn-danger btn-xs" onclick="safeDel('<?php echo ($item["id"]); ?>')">删除</buttton>
                    </td>
